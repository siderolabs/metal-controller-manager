...
func main() {
	...
	go func() {
		if err := tftp.ServeTFTP(); err != nil {
			setupLog.Error(err, "unable to start TFTP server", "controller", "MetalEnv")
		}
	}()
	...
	go func() {
		if err := ipxe.ServeIPXE(); err != nil {
			setupLog.Error(err, "unable to start iPXE server", "controller", "MetalEnv")
		}
	}()
